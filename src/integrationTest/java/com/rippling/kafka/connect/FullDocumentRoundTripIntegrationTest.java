...
import static com.rippling.kafka.connect.source.MongoSourceConfig.OUTPUT_JSON_FORMATTER_CONFIG;
...
import com.rippling.kafka.connect.source.MongoSourceConfig.ErrorTolerance;
...
public class FullDocumentRoundTripIntegrationTest extends MongoKafkaTestCase {
  ...
  void testRoundTripSimpleJsonFormat() {
    ...
    sourceProperties.put(
        ...
        "com.rippling.kafka.connect.source.json.formatter.SimplifiedJson");
    assertRoundTrip(
        IntStream.range(1, 100)
            .mapToObj(i -> BsonDocument.parse(format(FULL_DOCUMENT_JSON, i)))
            .collect(toList()),
  ...
  void testRoundTripBSON() {
    ...
    sinkProperties.put(
        ...
        "com.rippling.kafka.connect.sink.processor.BlockListValueProjector");
